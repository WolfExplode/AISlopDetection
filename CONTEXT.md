...
**4. Redundant Scaffolding** (structure dial ↑) — words about the content instead of content; saying it again. `pivot-paragraph`, `fractal-summaries`, `one-point-dilution`, `unnecessary-elaboration`, `superficial-analysis`, `false-conclusion`, `connector-addiction`, `unnecessary-contrast`, `listicle-instinct`, `listicle-trench-coat`, `bold-first-bullets`, `counterfeit-mechanism`. FP shape: the restatement carries new information (a summary that genuinely synthesizes).

Most of this mode is *transparent* redundancy — the restatement announces itself ("As we've seen…", a signpost, an eighth phrasing of the thesis). `counterfeit-mechanism` is the *opaque* sub-species and needs calling out separately: it restates while **upgrading the register**, so it reads as a new causal step rather than a repeat ("Food moves through quickly." → "The system is built for throughput."). The other mode-4 rules detect a surplus that is visibly there; this one detects a mechanism that is advertised and absent, which is why no client-side surface can reach it — the tell is only visible by comparing a sentence against its neighbour. It is the sentence-tier general case for register-upgraded restatement, the way `unearned-intensity` is for drama; `decorative-metaphor` is its crystallized client-side shape (restatement via an imported concrete image) and stays filed under mode 1, where the dial is drama rather than structure.
