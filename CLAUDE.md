...
1. **Fast pass** — `claude-haiku-4-5-20251001`, 30s timeout. Sentence and paragraph-level patterns (12 rules): throat-clearing, sycophantic frame, balanced take, unnecessary elaboration, empathy performance, pivot paragraph, grandiose stakes, historical analogy, false vulnerability, unearned intensity, counterfeit mechanism, false range (subtle cases).
...
- **LLM-required rules:** 14 (11 sentence-level + 3 document-level; `false-range` is a hybrid — client-side with an additional sentence-tier pass for subtle cases)
- **Total:** 78
